...
router.get('/today', async (req, res) => {
  try {
    ...
    const matches = await Match.find({ date:{$gte:today,$lt:tomorrow} }).populate('pronostic').sort({date:1}).limit(30);
    const sport = req.query.sport;
    const filtered = sport ? matches.filter(m => m.sport === sport) : matches;
    res.json(filtered.map(formatMatch));
...
function formatMatch(m) {
  return {
    ...
    status: m.status,
    home: { name: m.home.name, logo: m.home.logo, emoji: '🔵', score: m.home.score },
    away: { name: m.away.name, logo: m.away.logo, emoji: '🔴', score: m.away.score },
    odds: { h: m.odds?.h2h?.home, d: m.odds?.h2h?.draw, a: m.odds?.h2h?.away },
    prono: m.pronostic ? {
      tip: m.pronostic.tip, type: m.pronostic.type,
      cote: m.pronostic.cote, confiance: m.pronostic.confiance,
