...
async function getTodayFootballMatches() {
  try {
    ...
    const res = await sportsClient.get('/fixtures', {
      params: { date: today, timezone: 'Africa/Douala' },
    });

    return Promise.all(res.data.response.map(async fixture => ({
      ...
      home: { name: fixture.teams.home.name, logo: fixture.teams.home.logo, score: fixture.goals.home },
      away: { name: fixture.teams.away.name, logo: fixture.teams.away.logo, score: fixture.goals.away },
...
async function getTodayBasketballMatches() {
  try {
    ...
    const res = await axios.get('https://v1.basketball.api-sports.io/games', {
      ...
      params: { date: today, league: '12', season: '2024-2025' },
    });
...
async function getTodayTennisMatches() {
  try {
    ...
    return res.data.response.slice(0, 10).map(game => ({
      ...
      sport: 'tennis',
      league: game.tournament?.name || 'ATP/WTA',
      leagueLogo: game.tournament?.logo,
      date: new Date(game.date),
      status: mapStatus(game.status?.short || 'NS'),
      home: { name: game.players?.[0]?.name || 'Player 1', logo: game.players?.[0]?.flag },
      away: { name: game.players?.[1]?.name || 'Player 2', logo: game.players?.[1]?.flag },
    }));
  } catch (err) {
...
async function getMatchOdds(fixtureId) {
  try {
    const res = await sportsClient.get('/odds', { params: { fixture: fixtureId, bookmaker: '6' } });
...
module.exports = { getAllTodayMatches, getTodayFootballMatches, getMatchOdds };
